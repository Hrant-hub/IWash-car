...
export class CustomerSettingsController {
  ...
  @Post('cars')
  async addCar(@Req() req: any, @Body() body: AddCarDto) {
  ...
  @Post('locations')
  async addLocation(@Req() req: any, @Body() body: AddLocationDto) {
  ...
  async addCard(
    ...
    if (!/^\d{12,19}$/.test(cardNumber)) {
      throw new BadRequestException('cardNumber is invalid');
    }
    if (!holderName) {
    ...
    if (!Number.isInteger(expiryMonth) || expiryMonth < 1 || expiryMonth > 12 || !Number.isInteger(expiryYear)) {
      throw new BadRequestException('expiry is invalid');
    }
    if (!/^\d{3,4}$/.test(cvc)) {
      throw new BadRequestException('cvc is invalid');
    }
  ...
  async setDefaultCard(@Req() req: any, @Param('id') id: string) {
